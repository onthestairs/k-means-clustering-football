...
penalty_stats = ("Penalties Taken","Penalty Goals","Penalties Saved","Penalties Off Target","Penalties Not Scored")
set_piece_stats = ("Direct Free-kick Goals","Direct Free-kick On Target","Direct Free-kick Off Target","Blocked Direct Free-kick")
disallowed_stats = meta_stats + keeper_stats + corner_stats + foot_specific_stats + penalty_stats + set_piece_stats
...
class Player():
	...
	def calculate_normalised_stats(self, max_stats):
		self.normalised_stats = {k: (v/max_stats[k]) for k,v in self.stats.items() if max_stats[k] != 0}
...
def get_max_stats(players):
	max_stats = {k:0 for k in players.pop().stats.keys()}
	for p in players:
...
class kmeans():
	...
	def __init__(self, N, players, distance_function):
		self.N = N
	...
	def calculate(self, threshold=0.01):
		...
		new_total_distance = old_total_distance - threshold - 10
		count = 0
		while (old_total_distance - new_total_distance) > threshold:
			total = 0
			...
			count = count + 1
			print "Done iteration {0}. New distance is {1}. Old distance is {2}.".format(count, new_total_distance, old_total_distance)
...
players = parse(allowed=no_keepers)
print "parsed"

a = len(players)
players = [p for p in players if p.games_played > 15]
b = len(players)
